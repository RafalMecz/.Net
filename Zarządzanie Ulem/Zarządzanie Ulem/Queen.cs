            ...
            for(int i =0; i<workers.Length; i++)
            {
                ...
                if (String.IsNullOrEmpty(workers[i].CurrentJob))
                    ...
                    report += "Robotnica numer " + (i + 1) + " robi '" + workers[i].CurrentJob
                        + "' jeszcze przez " + workers[i].ShiftLeft + " zmiany \r\n";
                else
                    report += "Robotnica numer " + (i + 1) + " zakończy '" + workers[i].CurrentJob
                        + "' po tej zmianie \r\n";
            }

            report += "całkowite zużycie miodu: " + honeyConsumed + " jednostek \r\n";
            return report;
        }
    }
}
